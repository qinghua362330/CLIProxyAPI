...
type outerRetryIdentityError struct {
	retryAfter time.Duration
}

func (e *outerRetryIdentityError) Error() string   { return "rate limited" }
func (e *outerRetryIdentityError) StatusCode() int { return http.StatusTooManyRequests }
func (e *outerRetryIdentityError) RetryAfter() *time.Duration {
	d := e.retryAfter
	return &d
}

type outerRetryIdentityExecutor struct {
	id string
...
func (e *outerRetryIdentityExecutor) Identifier() string { return e.id }

func (e *outerRetryIdentityExecutor) recordIdentity(opts cliproxyexecutor.Options) (int, string) {
	e.mu.Lock()
	defer e.mu.Unlock()

	identity, _ := opts.Metadata[cliproxyexecutor.CodexRequestIdentityMetadataKey].(string)
	if identity == "" {
		identity = "identity-" + strconv.Itoa(len(e.identities)+1)
		opts.Metadata[cliproxyexecutor.CodexRequestIdentityMetadataKey] = identity
	}
	e.identities = append(e.identities, identity)
	return len(e.identities), identity
}

func (e *outerRetryIdentityExecutor) Execute(_ context.Context, _ *Auth, _ cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	call, _ := e.recordIdentity(opts)
	if call == 1 {
		return cliproxyexecutor.Response{}, &outerRetryIdentityError{retryAfter: time.Millisecond}
	}
	return cliproxyexecutor.Response{}, nil
}

func (e *outerRetryIdentityExecutor) ExecuteStream(_ context.Context, _ *Auth, _ cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	call, _ := e.recordIdentity(opts)
	if call == 1 {
		return nil, &outerRetryIdentityError{retryAfter: time.Millisecond}
	}
...
func (e *outerRetryIdentityExecutor) Refresh(_ context.Context, auth *Auth) (*Auth, error) {
	return auth, nil
}

func (e *outerRetryIdentityExecutor) CountTokens(context.Context, *Auth, cliproxyexecutor.Request, cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	return cliproxyexecutor.Response{}, &Error{HTTPStatus: http.StatusNotImplemented, Message: "CountTokens not implemented"}
}

func (e *outerRetryIdentityExecutor) HttpRequest(context.Context, *Auth, *http.Request) (*http.Response, error) {
	return nil, &Error{HTTPStatus: http.StatusNotImplemented, Message: "HttpRequest not implemented"}
}

func (e *outerRetryIdentityExecutor) identityCalls() []string {
	e.mu.Lock()
	defer e.mu.Unlock()
	out := make([]string, len(e.identities))
	copy(out, e.identities)
	return out
}

func newOuterRetryIdentityManager(t *testing.T, executor ProviderExecutor, model string) *Manager {
	t.Helper()

	m := NewManager(nil, nil, nil)
	m.SetRetryConfig(1, 50*time.Millisecond, 1)
	m.RegisterExecutor(executor)
	auth := &Auth{
		ID:       "outer-retry-identity-auth",
		Provider: "codex",
		Status:   StatusActive,
...
func TestManagerOuterRetryPreservesCodexRequestIdentity(t *testing.T) {
	const model = "gpt-5.5"
	tests := []struct {
		name   string
		...
		},
		{
			name: "execute_stream",
			invoke: func(m *Manager) error {
				_, err := m.ExecuteStream(context.Background(), []string{"codex"}, cliproxyexecutor.Request{Model: model}, cliproxyexecutor.Options{Stream: true})
				return err
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			executor := &outerRetryIdentityExecutor{id: "codex"}
			m := newOuterRetryIdentityManager(t, executor, model)
			if err := tt.invoke(m); err != nil {
				t.Fatalf("manager execution returned error: %v", err)
			}
			identities := executor.identityCalls()
			if len(identities) != 2 {
				t.Fatalf("executor calls = %d (%v), want 2", len(identities), identities)
			}
			if identities[0] == "" || identities[1] != identities[0] {
				t.Fatalf("request identities = %v, want the same non-empty ID across outer retries", identities)
			}
		})
	}
}
